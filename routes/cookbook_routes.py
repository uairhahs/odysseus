...
def setup_cookbook_routes() -> APIRouter:
    ...
    async def model_download(request: Request, req: ModelDownloadRequest):
        ...
        try:
            ...
            # TODO: better error handling / reporting if the assistant log is unavailable
            logger.warning("Assistant log unavailable")
            pass
    ...
    async def model_serve(request: Request, req: ServeRequest):
        ...
        try:
            ...
            logger.warning("Failed to log serve start to assistant", exc_info=True)
            pass
    ...
    async def list_gpus(
        ...
        try:
            ...
            logger.warning("Failed to get GPU processes from nvidia-smi", exc_info=True)
            pass
    ...
    def _maybe_sweep_orphans(tasks: list, state: dict) -> None:
        ...
        for srv in servers:
            ...
            try:
                ...
                logger.warning(f"orphan sweep: tmux ls failed on {host}", exc_info=True)
                continue
            for line in (ls.stdout or "").splitlines():
                ...
                try:
                    ...
                    logger.warning(
                        f"orphan sweep: failed to list panes for {sid} on {host}",
                        exc_info=True,
                    )
                    cur = []
                LIVE_PROCS = {
                ...
                m_model = re.search(r"model\s+(\S+)", pane)
                model = m_model.group(1) if m_model else ""
                if not model:
                    m_serve = re.search(r"vllm\s+serve\s+(\S+)", pane)
                    model = m_serve.group(1) if m_serve else f"adopted:{sid}"
                m_port = re.search(r"--port\s+(\d+)", pane)
        ...
        if adopted_any:
            try:
                ...
                logger.warning(f"orphan sweep: state write failed: {e}", exc_info=True)
    ...
    def _cookbook_tasks_status_sync():
        ...
        if _cookbook_state_path.exists():
            try:
                ...
                logger.warning(
                    "Failed to load cookbook state for tasks status check",
                    exc_info=True,
                )
                pass
            ...
            if local_win_task:
                ...
                try:
                    if log_path.exists():
                        ...
                        lines = [
                            ln.strip() for ln in full_snapshot.split("\n") if ln.strip()
                        ]
                        downloading_lines = [
                            ln for ln in lines if ln.startswith("Downloading")
                        ]
                    ...
                    logger.warning(
                        f"Failed to read log for {session_id}", exc_info=True
                    )
                    pass
            else:
                try:
                ...
                if is_alive:
                    try:
                        ...
                        # TODO: better error handling here, if the capture fails, we currently report "unknown" status with no output
                        # which isn't great for diagnosing issues with the capture itself (e.g. SSH failures, permission issues on Windows logs, etc.)
                        # At minimum we should log these exceptions and maybe include a hint in the UI that the status is unreliable due to capture issues.
                        logger.warning(
                            f"Failed to capture output for {session_id}", exc_info=True
                        )
                        pass
