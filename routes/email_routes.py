...
def _email_tag_owner_aliases(account_id: str | None, owner: str = "") -> list[str]:
    ...
    try:
        ...
        try:
            ...
            if not resolved_account_id:
                try:
                    ...
                    logger.debug(
                        f"Could not resolve default email account for owner '{owner}': {e}"
                    )
        ...
        logger.warning(f"Error gathering email samples: {e}")
    out = []
    for a in aliases:
        a = (a or "").strip()
        if a not in out:
    ...
    try:
        ...
        try:
            ...
            if count:
                ...
                rows = conn.execute(
                    f"SELECT message_key FROM email_event_seen "  # noqa: S608
...
def _list_imap_folders(conn) -> tuple[list, list[str]]:
    try:
        ...
        logger.debug(f"Could not list IMAP folders: {e}")
        return [], []
...
def _uid_exists(conn, uid: str) -> bool:
    try:
        ...
        logger.debug(f"UID existence check failed for UID '{uid}': {e}")
        return False
...
def _group_uid_fetch_records(msg_data) -> list:
    r"""Group an imaplib UID FETCH response into per-message (meta, payload).
...
def _sanitize_email_html(raw: str) -> str:
    ...
    try:
        ...
        logger.debug(f"Email HTML sanitization failed: {e}")
        return None
...
def setup_email_routes():
            ...
            if entry:
                ...
                if (now - last_used) < _IMAP_IDLE_MAX:
                    try:
                        ...
                        logger.debug(f"IMAP connection noop failed: {e}")
                        logger.infor("Attempting to recover by reopening connection")
                        try:
                            conn.logout()
                        except Exception as e:
                            logger.debug(f"Error logging out bad IMAP connection: {e}")
                        del _IMAP_POOL[pool_key]
                else:
                    try:
                        conn.logout()
                    except Exception as e:
                        logger.debug(f"Error logging out stale IMAP connection: {e}")
                    del _IMAP_POOL[pool_key]
        # Fresh connection
        ...
        try:
            ...
            if filter_ == "unread":
                ...
                try:
                    ...
                    if _tag_name == "spam":
                        rows_t = _ct.execute(
                            "SELECT message_id, uid FROM email_tags "  # noqa: S608
                        ...
                        rows_t = _ct.execute(
                            "SELECT message_id, uid, tags FROM email_tags "  # noqa: S608
                        ...
                        for r in rows_t:
                            try:
                                ...
                                logger.debug(f"Error processing tag row: {e}")
                    _ct.close()
            ...
            try:
                ...
                if _uid_strs:
                    ...
                    rows = _c.execute(
                        f"SELECT uid, tags, spam_verdict FROM email_tags "  # noqa: S608
                    ...
                    for r in rows:
                        try:
                            ...
                            logger.debug(f"Error loading tags for UID {r[0]}: {e}")
                            tg = []
                        if isinstance(tg, list):
                            tg = [
                                (
            ...
            if uid_list:
                ...
                try:
                    ...
                    if header_ids:
                        ...
                        rows_m = _cm.execute(
                            f"SELECT message_id, tags, spam_verdict FROM email_tags "  # noqa: S608
                        ...
                        for mid, tags_raw, spam_raw in rows_m:
                            try:
                                ...
                                logger.debug(
                                    f"Error loading tags for Message-ID {mid}: {e}"
                                )
                                tags = []
            ...
            try:
                ...
                if ids:
                    ...
                    rows = _c.execute(
                        f"SELECT message_id, summary FROM email_summaries "  # noqa: S608
    ...
    async def unflag_spam(uid: str, owner: str = Depends(require_owner)):
        ...
        try:
            ...
            _c = _sql3.connect(SCHEDULED_DB)
            _c.execute(
                f"UPDATE email_tags SET spam_verdict=0, spam_reason='' WHERE uid=? AND {owner_clause}",  # noqa: S608
                [uid, *owner_params],
            )
            _c.commit()
            _c.close()
    ...
    async def list_contacts(
        ...
        try:
            ...
            owner_clause, owner_params = _email_tag_owner_clause(None, owner)
            rows = conn.execute(
                f"SELECT sender FROM email_tags WHERE sender IS NOT NULL AND sender != '' AND {owner_clause}",  # noqa: S608
                owner_params,
            ).fetchall()
            conn.close()
            seen = {}
            for (s,) in rows:
                try:
                    ...
                    logger.debug(f"Error parsing contact {s}: {e}")
                    continue
                if not addr:
                    continue
                addr_l = addr.lower()
    ...
    def _read_email_sync(uid, folder, account_id, owner, mark_seen=True):
        r"""Sync IMAP read — wrapped in to_thread by the async handler.
        ...
        try:
            ...
            if mark_seen:
                ...
                try:
                    ...
                    logger.debug(f"mark-seen after read failed uid={uid}: {e}")
                    pass
            ...
            try:
                ...
                owner_clause, owner_params = _email_cache_owner_clause(owner)
                _row = _c.execute(
                    f"SELECT summary FROM email_summaries WHERE message_id = ? AND {owner_clause}",  # noqa: S608
                    (message_id.strip(), *owner_params),
                ).fetchone()
                if _row:
                    cached_summary = _row[0]
                _row2 = _c.execute(
                    f"SELECT reply FROM email_ai_replies WHERE message_id = ? AND {owner_clause}",  # noqa: S608
                    (message_id.strip(), *owner_params),
                ).fetchone()
                if _row2:
                ...
                try:
                    ...
                    logger.warning(
                        f"Error fetching sender signature for {sender_addr}: {e}"
                    )
                if _row3:
                    ...
                    if _row3[2]:
                        try:
                            ...
                            logger.warning(
                                f"Error parsing cached turns for {message_id}: {e}"
                            )
                            cached_turns = None
                _c.close()
            except Exception as e:
                logger.warning(f"Error fetching email boundaries for {message_id}: {e}")
    ...
    def _schedule_recent_email_warm(
        ...
        for em in emails:
            ...
            try:
                ...
                logger.warning(f"Error parsing date_epoch for {uid}: {e}")
                epoch = 0
            ...
            try:
                ...
                logger.warning(f"Error parsing size for {uid}: {e}")
                size = 0
        ...
        try:
            ...
            if ext in (".txt", ".md", ".markdown"):
                try:
                    ...
                    logger.warning(f"Failed to read text file {filepath.name}: {e}")
    ...
    async def mark_unread(
        ...
        r"""Mark an email as unread (clear \Seen flag)."""
        try:
            with _imap(account_id, owner=owner) as conn:
                conn.select(_q(folder))
                if not _store_email_flag(conn, uid, r"\Seen", add=False):
    ...
    async def mark_read(
        ...
        r"""Mark an email as read (set \Seen flag)."""
        try:
            with _imap(account_id, owner=owner) as conn:
                conn.select(_q(folder))
                if not _store_email_flag(conn, uid, r"\Seen", add=True):
                    return {"success": False, "error": "Email not found"}
            _invalidate_list_cache(account_id, folder)
            return {"success": True}
        except Exception as e:
    ...
    async def delete_email_permanent(
        ...
        try:
            with _imap(account_id, owner=owner) as conn:
                conn.select(_q(folder))
                if not _store_email_flag(conn, uid, r"\Deleted", add=True):
                    return {"success": False, "error": "Email not found"}
                conn.expunge()
            _invalidate_list_cache(account_id, folder)
            return {"success": True}
    ...
    async def mark_answered(
        ...
        r"""Mark an email as answered (set \Answered flag)."""
        try:
            with _imap(account_id, owner=owner) as conn:
                conn.select(_q(folder))
                if not _store_email_flag(conn, uid, r"\Answered", add=True):
                    return {"success": False, "error": "Email not found"}
            return {"success": True}
        except Exception as e:
    ...
    async def clear_answered(
        ...
        r"""Clear the \Answered flag from an email."""
        try:
            with _imap(account_id, owner=owner) as conn:
                conn.select(_q(folder))
                if not _store_email_flag(conn, uid, r"\Answered", add=False):
                    return {"success": False, "error": "Email not found"}
            return {"success": True}
        except Exception as e:
                    ...
                    with _imap(_account_id, owner=owner) as imap:
                        ...
                        if not sent_uid:
                            try:
                                ...
                                logger.warning(
                                    f"Failed to fetch UID for message {mid}: {e}"
                                )
                            ...
                            try:
                                ...
                                for folder_name in dict.fromkeys(folder_candidates):
                                    try:
                                        ...
                                        logger.warning(
                                            f"Failed to mark source as answered in folder {folder_name}: {e}"
                                        )
                                        continue
                            except Exception as e:
                                logger.warning(
        ...
        try:
            ...
            if message_id:
                try:
                    ...
                    owner_clause, owner_params = _email_cache_owner_clause(owner)
                    _row = _c.execute(
                        f"SELECT reply, model_used FROM email_ai_replies WHERE message_id = ? AND {owner_clause}",  # noqa: S608
                        ...
                        for _ in range(3):
                            if isinstance(_h, str):
                                try:
                                    ...
                                    logger.warning(
                                        f"Failed to decode session headers: {e}"
                                    )
                                    _h = None
                                    break
                            else:
            ...
            try:
                ...
                logger.warning(f"Error resolving utility endpoint: {e}")
            ...
            try:
                ...
                logger.warning(f"Error resolving default endpoint: {e}")
    ...
    async def update_email_config(data: dict, owner: str = Depends(require_owner)):
        ...
        db = SessionLocal()
        try:
            q = db.query(EmailAccount).filter(EmailAccount.is_default)  # noqa: E712
            if owner:
    ...
    async def get_email_urgency_state(owner: str = Depends(require_user)):
        ...
        try:
            ...
            logger.warning(f"Error loading email urgency state for {owner}: {e}")
            return {"total_unread": 0, "total_urgent": 0, "max_score": 0, "per_uid": {}}
    ...
    async def test_account_config(req: Request, owner: str = Depends(require_user)):
        ...
        try:
            ...
            logger.warning(f"Error parsing request body for account test: {e}")
