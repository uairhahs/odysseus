    ...
    if a >= 0 and b > a:
        ...
        for cand in (frag, _re.sub(r",(\s*[}\]])", r"\1", frag)):
            try:
                ...
                logger.warning(f"Failed to parse JSON fragment: {e}")
                continue
...
async def _run_skill_test_job(
    ...
    if skills_manager is not None:
        ...
        try:
            ...
            logger.warning(f"Failed to set audit: {e}")
...
def _skill_duplicate_blocker(skills_manager, name: str, owner) -> Optional[str]:
    ...
    if keeper_name and keeper_name != cur_name:
        try:
            ...
            logger.warning(f"Failed to set necessity: {e}")
        return keeper_name
    return None
...
def _audit_finalize_status(
    ...
    try:
        ...
        logger.warning(f"Failed to update skill status: {e}")
    return status
    ...
    try:
        for nm in names:
            ...
            try:
                ...
                logger.warning(f"Failed to refresh skill state: {e}")
...
def _resolve_audit_models(owner=None):
    ...
    try:
        ...
        logger.warning(f"Failed to resolve audit models: {e}")
...
def setup_skills_routes(skills_manager: SkillsManager) -> APIRouter:
    ...
    async def set_builtin_override(name: str, request: Request):
        ...
        from src.settings import load_settings, save_settings  # , get_setting
