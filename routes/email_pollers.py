...
def _latest_inbox_fallback_uids(conn, reconnect):
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Failed to logout from IMAP connection: {e}")
        return [], reconnect()
    ...
    try:
        ...
        if auto_cal:
            for sent_name in ("Sent", "INBOX/Sent", "Sent Items", "[Gmail]/Sent Mail"):
                try:
                    ...
                    logger.warning(f"Failed to select folder {sent_name}: {e}")
                    continue
        for folder in folders_to_scan:
            try:
                ...
                logger.warning(f"Folder {folder} scan failed: {e}")
        ...
        _sum_existing = {
            r[0]
            for r in _c.execute(
                f"SELECT message_id FROM email_summaries WHERE {_cache_owner_clause}",  # noqa: S608
                _cache_owner_params,
            ).fetchall()
        }
        _reply_existing = {
            r[0]
            for r in _c.execute(
                f"SELECT message_id FROM email_ai_replies WHERE {_cache_owner_clause}",  # noqa: S608
                _cache_owner_params,
            ).fetchall()
        }
        ...
        _cal_existing = (
            {
                r[0]
                for r in _c.execute(
                    f"SELECT message_id FROM email_calendar_extractions WHERE {_cache_owner_clause}",  # noqa: S608
                    _cache_owner_params,
                ).fetchall()
            }
            if auto_cal
        ...
        _urgent_existing = (
            {
                r[0]
                for r in _c.execute(
                    f"SELECT message_id FROM email_urgency_alerts WHERE {_cache_owner_clause}",  # noqa: S608
                    _cache_owner_params,
                ).fetchall()
            }
            if auto_urgent
                                    ...
                                    for op in ops[:3]:
                                        ...
                                        if action == "cancel":
                                            ...
                                            try:
                                                ...
                                                logger.warning(
                                                    f"Failed to process tracking links: {e}"
                                                )
                                            cal_args = json.dumps(
                                                {
                                                    "action": "create_event",
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        rows = conn.execute(
            f"SELECT id, to_addr, cc, bcc, subject, body, in_reply_to, references_hdr, attachments, account_id, {kind_expr}, {owner_expr}"  # noqa: S608
            " FROM scheduled_emails"
            f" WHERE status = 'pending' AND send_at <= ?",
            (now_iso,),
        ).fetchall()
        conn.close()
