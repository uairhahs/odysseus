...
def setup_memory_routes(
    ...
    def get_memory_by_session(request: Request, session_id: str):
        ...
        try:
            ...
            logger.warning(f"Session {session_id} not found for memory timeline: {e}")
        ...
        if ep_id:
            ...
            try:
                ep = (
                    ...
                    .filter(ModelEndpoint.id == ep_id, ModelEndpoint.is_enabled)
                    .first()
                )
                if ep:
                    ...
                    if not model and ep.models:
                        try:
                            ...
                            logger.warning(
                                f"Failed to load models for endpoint {ep_id}: {e}"
                            )
                    if ep.api_key:
    ...
    async def import_memories_from_file(
        ...
        if session:
            try:
                ...
                raise HTTPException(
                    404, "Session not found — needed for LLM config"
                ) from e
        else:
        ...
        try:
            ...
            return {
                "suggestions": [
                    {"text": line, "category": "fact"} for line in lines[:20]
                ],
                "filename": filename,
            }
        except Exception as e:
