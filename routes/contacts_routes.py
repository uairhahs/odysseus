...
def _vunesc(value: str) -> str:
    r"""Reverse _vesc() — turn escaped vCard text back into the raw value.
    Order matters: handle n/, /; first, backslash-unescape last."""
    if not value:
        return value
    out = []
    i = 0
...
def _fetch_contacts(force=False):
    ...
    if not force and _contact_cache["fetched_at"]:
        age = (
            datetime.now(timezone.utc) - _contact_cache["fetched_at"]
        ).total_seconds()
        if age < 60:
