    ...
    class AuthMiddleware(BaseHTTPMiddleware):
        async def dispatch(self, request: Request, call_next):
            ...
            try:
                ...
                # TODO: this is a belt-and-suspenders check around the token compare_digest
                logger.warning(
                    "Failed to verify internal tool token: %s", e, exc_info=True
                )
            ...
            if auth_header.startswith("Bearer ody_"):
                ...
                try:
                    ...
                    if matched_id:
                        ...
                        async def _touch_last_used(tid: str):
                            ...
                            try:
                                ...
                                logger.warning(
                                    "Failed to update last_used_at for API token",
                                    exc_info=True,
                                )
                    ...
                    logger.warning("API token auth error: %s", e, exc_info=False)
...
async def serve_generated_image(filename: str, request: Request):
    ...
    try:
        # Deferred: src.auth_helpers is circular-import sensitive, must not be hoisted.
        ...
        if _user:
            ...
            try:
                _row = (
                    _db.query(GalleryImage)
                    .filter(GalleryImage.filename == filename)
                    .first()
                )
        ...
        # TODO: this is a belt-and-suspenders check around the DB query and ownership compare. If the DB is unreachable or there's some other error, we don't want to accidentally serve every generated image to anyone who can guess a filename.
        logger.warning("Failed to verify image ownership: %s", e, exc_info=True)
