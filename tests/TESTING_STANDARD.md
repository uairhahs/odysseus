...
This document defines _how we write and refactor tests_ in Odysseus. It is the
...
When the two ever disagree, this file states the _intent_ and `README.md` states
the _current mechanics_; fix whichever is stale.
...
toward _after_ helpers and determinism are stable. Until a category is moved,
...
- **Narrow exception** - a source-text/AST assertion is acceptable only when the
  ...
  When used, say _why_ in the test docstring so it is a deliberate choice, not a
  shortcut.
- Do not convert source-text assertions to behavioral ones in the _same_ PR that
...
- Prefer 3-6 files per refactor batch, and only when they share the _same_
  pattern.
...
Move toward this layout _gradually_, only after helper conventions and
