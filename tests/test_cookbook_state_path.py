    ...
    for rel in FILES:
        ...
        for ln in text.splitlines():
            if ln.strip().startswith("#"):
                continue
            assert (
                "/app/data/cookbook_state" not in ln
            ), f"{rel}: hardcoded /app/data: {ln.strip()}"
            assert (
                'os.environ.get("DATA_DIR"' not in ln
            ), f"{rel}: relative DATA_DIR env fallback: {ln.strip()}"
...
def test_cookbook_state_uses_datadir_constant():
    ...
    for rel in FILES:
        ...
        if "cookbook_state.json" in text:
            assert (
                "from core.constants import DATA_DIR" in text
            ), f"{rel}: missing DATA_DIR import"
