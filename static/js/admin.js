    ...
    data.users.forEach((u) => {
      ...
      if (!u.is_admin) {
        ...
        header.addEventListener("click", (e) => {
          if (e.target.closest(".admin-btn-delete, [data-adm-rename-user]"))
            return;
...
async function _loadModelsForUser(
  ...
  try {
    ...
    function _saveModels() {
      ...
      if (hint)
        hint.textContent = !restricted
          ? "All models allowed (no restrictions)"
          : value.length === 0
            ? "No models allowed"
            : value.length + " model(s) allowed";
...
function initEndpointForm() {
  ...
  el("adm-epAddBtn").addEventListener("click", async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (!d.online) {
          msg.textContent =
            "Added (endpoint offline — will retry on next load)";
          msg.className = "admin-error";
  ...
  if (copilotBtn) {
    ...
    copilotBtn.addEventListener("click", async () => {
      ...
      const reset = () => {
        ...
        copilotBtn.textContent = "Connect GitHub Copilot";
        copilotPolling = false;
      };
      status.textContent = "";
      status.className = "adm-ep-inline-msg";
      copilotBtn.disabled = true;
      copilotBtn.textContent = "Starting...";
      copilotPolling = true;
      let start;
      try {
        const res = await fetch("/api/copilot/device/start", {
          method: "POST",
          body: new FormData(),
          credentials: "same-origin",
        });
        ...
        if (!res.ok) {
          status.textContent = start.detail || "Failed to start login";
      ...
      status.innerHTML =
        ...
        encodeURI(authUrl) +
        '" target="_blank" rel="noopener">Authorize on GitHub ↗</a>' +
      ...
      const poll = async () => {
        if (Date.now() > deadline) {
          done("admin-error", "Authorization expired — try again.");
          return;
        }
        try {
          const fd = new FormData();
          fd.append("poll_id", poll_id);
          const r = await fetch("/api/copilot/device/poll", {
            method: "POST",
            body: fd,
            credentials: "same-origin",
          });
          ...
          if (d.status === "authorized") {
            ...
            if (d.endpoint && d.endpoint.id)
              _recentlyAddedEpId = String(d.endpoint.id);
            await loadEndpoints();
          ...
          if (d.status === "failed") {
              ...
              "admin-error",
              "Authorization failed (" + (d.error || "denied") + ").",
            );
            return;
          }
        } catch (e) {
  ...
  if (localAddBtn) {
    localAddBtn.addEventListener("click", async () => {
      ...
      localAddBtn.textContent = "Adding...";
      try {
        const fd = new FormData();
        fd.append("base_url", url);
        if (apiKey) fd.append("api_key", apiKey);
  ...
  if (discoverBtn) {
    discoverBtn.addEventListener("click", async () => {
      ...
      try {
        ...
        txt.textContent =
          "Scanning ports 8000-8020 and 11434 for model servers...";
      ...
      try {
        ...
        if (!items.length) {
          ...
          const parts = [
            `Found ${items.length} server${items.length !== 1 ? "s" : ""} with ${totalModels} model${totalModels !== 1 ? "s" : ""}`,
          ];
...
async function loadTokens() {
  ...
  try {
    ...
    if (!tokens.length) {
      list.innerHTML = '<div class="admin-empty">No API tokens</div>';
      return;
    }
...
function initTokenForm() {
  ...
  el("adm-tokenCopyBtn").addEventListener("click", () => {
    ...
    navigator.clipboard.writeText(val).then(() => {
      ...
      setTimeout(() => {
        el("adm-tokenCopyBtn").textContent = "Copy";
      }, 2000);
    });
  });
}
...
function initDangerZone() {
  ...
  modalEl.querySelectorAll("[data-wipe-kind]").forEach((btn) => {
    btn.addEventListener("click", async () => {
      ...
      if (
        !(await uiModule.styledConfirm(
          `Wipe ALL ${label}? This cannot be undone.`,
          { confirmText: "Wipe", danger: true },
        ))
      )
        return;
      if (
        !(await uiModule.styledConfirm(
          `Really wipe every one of your ${label}?`,
          { confirmText: "Yes, wipe everything", danger: true },
        ))
      )
        return;
      btn.disabled = true;
      ...
      try {
        ...
        if (res.ok) {
          if (_wipeMsg) {
            _wipeMsg.textContent = `Wiped ${data.count ?? 0} ${label}.`;
