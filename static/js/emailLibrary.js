...
function _syncEmailReadState(uid, isRead = true) {
  ...
  document
    ...
    .forEach((card) => {
      ...
      const rightCluster = titleRow.querySelector(
...
async function _prewarmDefaultEmailView() {
  ...
  const res = await fetch(
    `${API_BASE}/api/email/list?folder=${encodeURIComponent(folder)}${accountQS}&limit=100&offset=0&filter=${filter}`,
    {
      credentials: "same-origin",
    },
  );
...
async function _doSearch() {
  ...
  try {
    ...
    const stats = document.getElementById("email-lib-stats");
    if (stats)
      stats.textContent = `${data.total || results.length} match${(data.total || results.length) === 1 ? "" : "es"}`;
  } catch (e) {
    sp.destroy();
...
function _prefetchAdjacentEmails(card, count = 1) {
  ...
  _emailReadPrefetchTimer = setTimeout(() => {
    ...
    fetch(
      `${API_BASE}/api/email/read/${encodeURIComponent(uid)}?folder=${encodeURIComponent(state._libFolder)}${_acct()}&mark_seen=false`,
    )
      .catch(() => {})
...
async function _bulkAction(action) {
  ...
  if (action === "delete") {
    if (deleteBtn) {
      ...
      deleteBtn.innerHTML =
        '<span class="email-bulk-loading-label">Deleting</span>';
...
function _showAiReplyChoice(btn, em, data) {
  ...
  setTimeout(
    () => document.addEventListener("click", _closeAiReplyChoice, true),
    0,
  );
}
...
async function _createEmailReplyReminder(em, dueDate) {
  ...
  const payload = {
    title: `Reply: ${subject}`,
    note_type: "todo",
    items: [{ text: `Reply to ${from}: ${subject}`, checked: false }],
