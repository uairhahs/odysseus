...
function _rerenderCachedModels() {
  ...
  list.querySelectorAll(".memory-item[data-repo]").forEach((item) => {
    item.addEventListener("click", (e) => {
      ...
      const _allowedBackends = new Set(
        _isWindows()
          ? ["llamacpp"]
          : _isMetal()
            ? ["llamacpp", "ollama"]
            : ["vllm", "sglang", "llamacpp", "ollama", "diffusers"],
      );
      const defaultBackend =
        ss._forceBackend && ss.backend && _allowedBackends.has(ss.backend)
          ? ss.backend
          : detectedBackend;
      const savedMatchesBackend =
        !!ss._forceBackend || (ss.backend || "vllm") === detectedBackend;
      const sv = (k, def) =>
      ...
      const vllmKvCacheOpts = ["auto", "fp8"]
        .map(
          (d) =>
            `<option value="${d}"${sv("vllm_kv_cache_dtype", "auto") === d ? " selected" : ""}>${d}</option>`,
        )
        .join("");
      const _l = (name, tip) =>
      ...
      const vllmAttnBackendOpts = [
        ...
        .map(
          (b) =>
            `<option value="${b === "auto" ? "" : b}"${sv("vllm_attn_backend", "") === (b === "auto" ? "" : b) ? " selected" : ""}>${b}</option>`,
        )
        .join("");
      ...
      panel
        ...
        .addEventListener("click", async (ev) => {
          ...
          const launchCmd = String(_rawLaunchCmd || "")
            .replace(/\s+/g, " ")
            .trim();
          if (_cmdTextarea && _cmdTextarea.value !== launchCmd)
            _cmdTextarea.value = launchCmd;
          const serveState = {};
          panel.querySelectorAll(".hwfit-sf").forEach((el) => {
            if (el.type === "checkbox")
          ...
          try {
            ...
            localStorage.setItem(
              SERVE_STATE_KEY,
              JSON.stringify({ _byRepo: byRepo, _lastUsed: _saved }),
            );
          } catch {}
...
export async function openServePanelForRepo(repo, fields) {
  ...
  if (fields && typeof fields === "object") {
    try {
      ...
      localStorage.setItem(
        SERVE_STATE_KEY,
        JSON.stringify({ _byRepo: byRepo, _lastUsed: _seeded }),
      );
    } catch {}
  }
