...
function _canClearTask(task) {
  ...
  return ["done", "stopped", "error", "crashed", "failed"].includes(
    task.status,
  );
}
...
function _outputEndsInPipPreparingSpinner(output) {
  ...
  return /Preparing\s+\w+\s+for\s+collected\s+packages|\[\d+\/\d+\]|\(\d+\/\d+\)|░|█/i.test(
    tail,
  );
}
...
function _loadTombstones() {
  try {
    return JSON.parse(localStorage.getItem(_REMOVED_KEY)) || {};
  } catch {
    return {};
  }
}
...
export function _saveTasks(tasks) {
  localStorage.setItem(
    TASKS_KEY,
    JSON.stringify((tasks || []).map(_stripTaskSecrets)),
  );
  _syncToServer();
}
...
function _winSessionCmd(task, tmuxArgs) {
  ...
  if (tmuxArgs.includes("capture-pane")) {
    ...
    return host
      ? `ssh ${pf}${host} "powershell -Command \\"${ps}\\""`
...
export async function _syncFromServer() {
  try {
    ...
    localStorage.setItem(
      TASKS_KEY,
      JSON.stringify(merged.map(_stripTaskSecrets)),
    );
...
async function _openServeEditForTask(task, cmdOverride, fieldOverrides = null) {
  ...
  if (_tSrv) {
    ...
    _envState.platform = _tSrv.platform || "";
  } else if (!_tHost) {
    _envState.env = "none";
    _envState.envPath = "";
    _envState.platform = "";
...
export function _renderRunningTab() {
  ...
  if (hasContent && !runTab) {
    ...
    runTab.innerHTML = tasks.length
      ? `Running${activeCountHtml}${_errCount2 ? '<span class="cookbook-tab-error-dot"></span>' : ""}`
      : "Running";
    if (!hasContent) {
  ...
  if (hasContent && !group) {
    ...
    group.innerHTML =
      ...
      '<h2 style="margin:0;padding:0;line-height:1;">Running <span id="running-count" class="memory-count" style="font-size:0.6em;opacity:0.6;font-weight:normal">' +
      activeCount +
      "</span></h2>" +
      "</div>" +
  ...
  for (const t of tasks) {
    ...
    if (!serverGroups[key])
      serverGroups[key] = { name: _serverName(key), serve: [], download: [] };
    ...
    btn.addEventListener("click", async (e) => {
      ...
      if (
        !(await window.styledConfirm(
          `Clear finished tasks on ${_serverName(host)}?`,
          { confirmText: "Clear" },
        ))
      )
        return;
      ...
      const toRemove = allTasks.filter(
        (t) => (t.remoteHost || "") === host && _canClearTask(t),
      );
  ...
  group.querySelectorAll("[data-stop-server]").forEach((btn) => {
    ...
    btn.addEventListener("click", async (e) => {
      ...
      const running = _loadTasks().filter(
        (t) => (t.remoteHost || "") === host && t.status === "running",
      );
      if (!running.length) {
    ...
    if (task.type === "download") {
      ...
      if (_serveBtn) {
        _serveBtn.addEventListener("click", async (e) => {
          ...
          try {
            const { openServePanelForRepo } =
              await import("./cookbookServe.js");
    ...
    if (menuBtn) {
      ...
      menuBtn.addEventListener("click", (e) => {
        ...
        if (task.type === "serve" && task.payload?.repo_id) {
          items.push({
            label: "Edit in serve panel",
            action: "edit-panel",
            tooltip:
              "Open the full Serve config panel pre-filled with this task — pick a different backend, change GPUs, edit env vars, then Launch from there",
            custom: () => _openEdit(),
          });
        }
        ...
        if (
          ...
          items.push({
            label: "Edit cmd & relaunch",
            action: "edit",
            tooltip:
              "Edit the raw vllm/llama-server cmd string in a dialog and relaunch immediately on the same host",
            custom: async () => {
        ...
        if (_isWindows(task)) {
          ...
          const sd = host
            ...
            : "$env:TEMP\\odysseus-tmux";
          const logCmd = host
            ? `ssh ${_sshPrefix(_getPort(task))}${host} "powershell -Command \\"Get-Content '${sd}\\${task.sessionId}.log' -Tail 50\\""`
            : `powershell -Command "Get-Content (Join-Path $env:TEMP 'odysseus-tmux\\${task.sessionId}.log') -Tail 50"`;
          items.push({
            label: "Copy log cmd",
            action: "copy-tmux",
            custom: () => {
  ...
  while (!controller.signal.aborted) {
    ...
    try {
      ...
      if (data.exit_code !== 0) {
        ...
        // fails transiently but the task is alive. Fast cache-hit installs that
        // finish before the first poll are handled by the empty-output-done path.
        ...
        const _isPipTaskDiag =
          ...
          (task.payload?.repo_id || "").startsWith("pip-") ||
          /python3? -m pip\b/.test(task.payload?._cmd || "") ||
          /\buv pip\b/.test(task.payload?._cmd || "");
        ...
        if (diag) {
          ...
          const endsPipPreparingPhase =
            (task.payload?._dep || _isPipTask) &&
            _outputEndsInPipPreparingSpinner(lastOutput);
          // A _dep task (uv pip install) can finish so fast on a cache hit that
          // the tmux session is already dead before the first capture-pane runs,
          ...
          if (endsPipPreparingPhase) {
            // Still in a quiet pip phase — back off and let the next poll cycle decide.
            await new Promise((r) => setTimeout(r, 5000));
            failCount = 0;
            continue;
          }
...
function _refreshServerDots() {
  ...
  const byKey = {};
  for (const t of tasks) {
    (byKey[t.remoteHost || ""] = byKey[t.remoteHost || ""] || []).push(t);
  }
...
async function _probeEndpointUntilOnline(epId, host, port) {
  ...
  for (let i = 0; i < MAX_TRIES; i++) {
    ...
    try {
      ...
      await fetch(`/api/model-endpoints/${epId}/probe`, {
        credentials: "same-origin",
      })
        .then((r) => r.text())
        .catch(() => {});
