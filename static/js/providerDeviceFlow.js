...
async function _fetchJson(fetchImpl, url, options, fallback) {
  ...
  if (!response.ok) {
    throw new Error(
      _messageFromPayload(
        payload,
        fallback || `Request failed (HTTP ${response.status})`,
      ),
    );
  }
  return payload;
...
export async function runProviderDeviceFlow(provider, options = {}) {
  ...
  if (!start.poll_id)
    throw new Error(`${cfg.label} sign-in did not return a poll id`);
  const authUrl = cfg.authUrl(start);
  await _callCallback(options.onStart, {
    provider,
    config: cfg,
    start,
    authUrl,
  ...
  while (true) {
    ...
    await _callCallback(options.onWaiting, {
      provider,
      config: cfg,
      start,
      authUrl,
