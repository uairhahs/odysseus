...
function _getPort(hostOrTask) {
  ...
  if (typeof hostOrTask === "object")
    return hostOrTask.sshPort || _getPort(hostOrTask.remoteHost);
...
function _detectModelOptimizations(modelName) {
  ...
  if (
    ...
    (n.includes("deepseek") &&
      (n.includes("v3") || n.includes("v3.1") || n.includes("r1"))) ||
    n.includes("kimi-k2") ||
    n.includes("kimi_k2") ||
    n.includes("glm-4.5") ||
    n.includes("glm4.5") ||
...
export function _buildServeCmd(f, modelName, backend) {
  ...
  const _venvBin = _isVenv
    ? _envState.envPath.replace(/\/+$/, "") + "/bin/"
    : "";
  ...
  if (backend === "vllm") {
    ...
    if (f.reasoning_parser) {
      const rp =
        typeof f.reasoning_parser === "string" && f.reasoning_parser !== "true"
          ? f.reasoning_parser
          : f._reasoning_parser_value || "qwen3";
    ...
    const lcPrefix = (() => {
      let p = "";
      if (f.unified_mem && !_cpuOnly && !_isWindows())
  ...
  try {
    ...
    async function _installDep(
      ...
      if (_isLocalServer) {
        ...
        uiModule.showToast(
          `${remove ? "Removing" : upgrade ? "Updating" : "Installing"} ${pkgName} (uv pip ${remove ? "uninstall" : "install"})...`,
        );
        try {
          ...
          if (data.session_id) {
            const payload = {
              repo_id: pipName,
              _cmd: `uv pip install --link-mode=copy ${pipName}`,
              remote_host: "",
              _dep: true,
              env_path: "",
            };
            ...
            uiModule.showToast(
              `${remove ? "Removed" : upgrade ? "Updated" : "Installed"} ${pkgName} in project dependencies.`,
            );
          ...
          uiModule.showToast(
            `${remove ? "Remove" : "Install"} failed: ${err.message}`,
          );
          if (statusEl) {
            statusEl.textContent = remove
              ? "Remove"
      ...
      if (remove) {
        uiModule.showToast(
          "Remove is only supported for local project dependencies right now.",
        );
        return;
      }
    ...
    function _showDepMenu(anchor) {
      ...
      const rmIco =
        '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18"/><path d="M8 6V4h8v2"/><path d="M19 6l-1 14H6L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/></svg>';
...
function _wireTabEvents(body) {
    ...
    rebuildBtn.addEventListener("click", async () => {
      ...
      try {
        ...
        if (!res.ok || !data.ok) {
          ...
          uiModule.showToast(
            `Cleared llama.cpp build on ${where}. Re-launch the serve task to rebuild with GPU support.`,
          );
        }
      } catch (err) {
    ...
    document.addEventListener(
      ...
      async (ev) => {
        ...
        _launchServeTask(
          `reinstall-${pkg}`,
          "pip-reinstall",
          `(uv pip install --force-reinstall --no-deps ${pkg} || ${_venvPy} -m pip install --force-reinstall --no-deps ${pkg})`,
        );
      },
      true,
    );
  ...
  if (dlBtn && dlInput) {
    ...
    const triggerDownload = () => {
      ...
      if (!/^[^\s/]+\/[^\s/]+$/.test(repo)) {
        uiModule.showToast(
          'Enter a full HuggingFace repo ID like "org/model-name" (or paste the full HF URL).',
        );
        dlInput.focus();
        return;
      }
  ...
  if (dlFold && dlFoldBody && dlFoldChevron) {
    dlFold.addEventListener("click", () => {
      ...
      try {
        localStorage.setItem("cookbook_dl_tab_folded_v1", folded ? "0" : "1");
      } catch {}
    });
  }
...
function _renderRecipes() {
  ...
  html +=
    '<button type="button" class="hwfit-gpu-btn" id="hwfit-rescan" title="Re-scan hardware" style="flex-shrink:0;position:relative;top:-3px;left:-1px;">↻ RESCAN</button>';
  html +=
  ...
  html +=
    '<div class="hwfit-list-footer" style="margin-top:8px;padding-top:6px;border-top:1px solid color-mix(in srgb, var(--border) 50%, transparent);font-size:9.5px;opacity:0.65;text-align:right;">' +
    "Don't see a model? " +
