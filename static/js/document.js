...
async function _aiReply() {
  ...
  try {
    ...
    if (data.success && data.reply) {
      ...
      const quoteIdx = lines.findIndex(
        (l) => l.startsWith("On ") && l.includes(" wrote:"),
      );
      let newBody = "";
      if (quoteIdx > 0) {
    ...
    if (btn) {
      btn.disabled = false;
      btn.innerHTML =
        '<svg width="11" height="11" viewBox="0 0 24 24" fill="currentColor" style="vertical-align:-1px;margin-right:3px"><path d="M12 0L14.59 8.41L23 12L14.59 15.59L12 24L9.41 15.59L1 12L9.41 8.41Z"/></svg>AI Reply';
    }
  }
}
...
export function openPanel() {
  ...
  document
    ...
    ?.addEventListener("click", _aiReply);
...
function initMdToolbar() {
  ...
  toolbar.addEventListener("mousedown", (e) => {
    if (e.target.closest("[data-md], .md-dd-toggle, .emoji-picker-btn"))
      e.preventDefault();
  });
