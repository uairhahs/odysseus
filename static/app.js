  ...
  if (sortBtn && sortDropdown) {
    ...
    function _syncSortChecks() {
      ...
      sortDropdown.querySelectorAll(".sort-option").forEach((o) => {
        ...
        check.style.cssText =
          "float:right;font-size:20px;line-height:1;position:relative;top:3px;color:var(--accent, var(--red));opacity:" +
        ...
        if (data.status === "ok") {
          ...
          if (skipLlm) {
            ...
            if (data.updated > 0) {
              ...
              if (remaining > 0)
                msg += ` — ${remaining} unfiled left, hit Tidy again`;
            } else if (remaining > 0) {
    ...
    if (autoSortMoreBtn && autoSortNoaiBtn) {
      autoSortMoreBtn.addEventListener("click", (e) => {
        e.stopPropagation();
        autoSortNoaiBtn.style.display =
          autoSortNoaiBtn.style.display === "none" ? "block" : "none";
      });
